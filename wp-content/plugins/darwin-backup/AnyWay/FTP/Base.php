 ...
 *
 */
abstract class AnyWay_FTP_Base {
    /* Public variables */
    var $LocalEcho;
    var $Verbose;
    var $OS_local;
    ...
    function __construct($port_mode=FALSE, $verb=FALSE, $le=FALSE) {
        ...
        $this->AutoAsciiExt=array('ASP','BAT','C','CPP','CSS','CSV','JS','H','HTM','HTML','SHTML','INI','LOG','PHP','TPL','PHTML','PL','PERL','SH','SQL','TXT');
...
// <!-- --------------------------------------------------------------------------------------- -->
    ...
    function connect($server=NULL) {
        ...
        if(!($this->_ftp_control_sock = $this->_connect($this->_host, $this->_port))) {
            $this->SendMSG(sprintf("Error : Can't connect to remote host '%s :%s'", $this->_fullhost,$this->_port));
            return FALSE;
        }
        $this->SendMSG(sprintf("Connected to remote host '%s:%s'. Waiting for greeting.", $this->_fullhost,$this->_port));
        do {
    ...
    function mput($local='.', $remote=NULL, $continious=false) {
        ...
        if(!@file_exists($local)) {
            $this->PushError('mput',"can't open local folder", "Can't stat folder '{$local}'");
            return FALSE;
        }
        ...
        if($handle = opendir($local)) {
            ...
            while (false !== ($file = readdir($handle))) {
                if (!in_array($file,array('.','..'))) $list[]=$file;
            }
            closedir($handle);
        } else {
            $this->PushError('mput',"can't open local folder", "Can't read folder '{$local}'");
            return FALSE;
        }
    ...
    function mget($remote, $local='.', $continious=false) {
        ...
        if($list===false) {
            $this->PushError('mget',"can't read remote folder list", sprintf("Can't read remote folder '%s' contents", $remote));
            return FALSE;
        }
        if(empty($list)) return true;
        if(!@file_exists($local)) {
        ...
        foreach($list as $k=>$v) {
            ...
            if(in_array($list[$k]['name'],array('.','..'))) unset($list[$k]);
        }
        $ret=true;
        foreach($list as $el) {
            $info = array($remote, $el['name'], $local, $el['name']);
            if($el['type']=='d') {
                if(!$this->mget($remote.'/'.$el['name'], $local.'/'.$el['name'], $continious)) {
                    $this->PushError('mget', "can't copy folder", vsprintf("Can't copy remote folder '%s/%s' to local '%s/%s'",$info));
                    $ret=false;
                    if(!$continious) break;
                }
            } else {
                if(!$this->get($remote.'/'.$el['name'], $local.'/'.$el['name'])) {
                    $this->PushError('mget', "can't copy file", vsprintf("Can't copy remote file '%s/%s' to local '%s/%s'",$info));
                    $ret=false;
                    if(!$continious) break;
                }
            }
...
// Rcupre une erreur externe
    ...
    abstract function _data_prepare($mode = FTP_ASCII);

    abstract function _data_read($mode = FTP_ASCII, $fp = NULL);

    abstract function _data_write($mode = FTP_ASCII, $fp = NULL);

    abstract function _data_close();

    abstract function _readmsg($fnction = "_readmsg");

    abstract function _exec($cmd, $fnction = "_exec");

    abstract function _connect($host, $port);

    abstract function _settimeout($sock);
}
