(function ($, window) {
    ...
    window.app = {
        retries: 0,
        init: function init(button, ajaxurl) {
            var self = this,
                ref = $(button).ladda(),
                ladda = ref.ladda.bind(ref);
        ...
        start: function start() {
            ...
            $.post(self.ajaxurl, request, function (response) {
                if (response && response.success) {
                    ...
                    self.ladda('start');
                    self.ladda('setProgress', 0);
                    self.next();
                } else if (response) {
                    $(self).trigger("error", null, response.data.message);
                }
            });
        },
        next: function () {
            ...
            if (self.sid) {
                ...
                self.xhr = $.post(self.ajaxurl, request, function (response) {
                    self.xhr = null;
                    if (response && response.success) {
                        self.retries = 0;
                        ...
                        $(self).trigger("error", response.data.message);
                    } else {
                        if (self.retries++ <= 10) {
                            self.next();
                        } else {
                            if (window.Raven)
                                window.Raven.captureMessage(response)
                            $(self).trigger("error", response.responseText);
                        }
                    }
                });
            }
        ...
        setButtonTextFromAttr: function (attr) {
            var text;
            if (text = this.$button.attr(attr))
                this.$button.find('.ladda-label').html(text);
            else
                this.$button.find('.ladda-label').html(attr);
        },
        setButtonHandler: function (handler) {
            this.$button.unbind("click");
            this.$button.bind("click", handler);
        },
        setTracking: function () {
            tracker.setTracking.apply(tracker, Array.prototype.slice.apply(arguments))
        },
        track: function () {
            tracker.track.apply(tracker, Array.prototype.slice.apply(arguments))
        }
    };
    ...
    $(app).bind("progress", function progress(e, progress) {
        this.ladda('setProgress', progress / 100);
        this.next();
    });

    $(app).bind("done", function done(e, link) {
        this.ladda('stop');
    });
